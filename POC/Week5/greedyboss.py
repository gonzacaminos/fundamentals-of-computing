...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    while current_day <= days_in_simulation:
        earnings += salary
        money += salary
        print "Day:", current_day, "Money:", money, "Earnings:", earnings, "Salary:", salary, "Bribe:",bribe, int(math.floor(earnings/bribe))
        
        for i in range(int(money/bribe)): 
            # Bribe day!
        ...
        days_to_skip = math.ceil((bribe - money)/salary)
        current_day += days_to_skip
        # We take one salary out of the salary, it will be added later
        skipped_salary = salary * (days_to_skip - 1)
        
        earnings += skipped_salary
        money += skipped_salary
